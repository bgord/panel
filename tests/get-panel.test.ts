...
describe(`GET ${url}`, async () => {
  ...
  test("happy path - pl", async () => {
    ...
    using _ = spyOn(di.Adapters.System.RemoteFileStorage, "getStream").mockResolvedValue(mocks.stream());

    const response = await server.request(
      url,
      { method: "GET", headers: { "Accept-Language": "pl" } },
      mocks.ip,
    );
    ...
    expect(response.headers.get("Last-Modified")).toEqual(mocks.T0Date);
    expect(response.headers.get("Cache-Control")).toEqual("private, max-age=0, must-revalidate");
    expect(remoteFileStorageHead).toHaveBeenCalledWith(mocks.objectKeyPl);
  });

  test("happy path - fallback", async () => {
    using remoteFileStorageHead = spyOn(di.Adapters.System.RemoteFileStorage, "head").mockResolvedValue(
      mocks.head,
    );
    using _ = spyOn(di.Adapters.System.RemoteFileStorage, "getStream").mockResolvedValue(mocks.stream());
    ...
    const response = await server.request(
      url,
      { method: "GET", headers: { "Accept-Language": "fr" } },
      mocks.ip,
    );
    ...
    expect(response.headers.get("Last-Modified")).toEqual(mocks.T0Date);
    expect(response.headers.get("Cache-Control")).toEqual("private, max-age=0, must-revalidate");
    expect(remoteFileStorageHead).toHaveBeenCalledWith(mocks.objectKey);
  });
});
